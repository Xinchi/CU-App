...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if (trimmedComment.length != 0 && [self.photo objectForKey:kPAPPhotoUserKey]) {
        ...
        [comment saveEventually:^(BOOL succeeded, NSError *error) {
            MyLog(@"Comment posted, Succesfull = %d", succeeded);
            [timer invalidate];
            ...
            if (error && [error code] == kPFErrorObjectNotFound) {
                ...
                // TODO: A temporary solution to dimiss the HUD before Remote Notification is done.
                // When Remote Notification has been implemented, remove this 2 lines
                ...
                if (channelSet.count > 0) {
                    NSString *alert = [NSString stringWithFormat:@"%@: %@", [PAPUtility firstNameForDisplayName:[[User currentUser] objectForKey:kPAPUserDisplayNameKey]], trimmedComment];
                    ...
                    [push setData:data];
                    MyLog(@"TODO: Push Notification Here");
...
- (void)handleCommentTimeout:(NSTimer *)aTimer {
    MyLog(@"handleCommentTimeout fired");
    [MBProgressHUD hideHUDForView:self.view.superview animated:YES];
